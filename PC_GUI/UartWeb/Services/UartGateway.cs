...
public sealed class UartGateway : IDisposable
{
    ...
    /* SSE 텔레메트리 브로드캐스트: 수신 즉시 연결된 모든 클라이언트에 푸시 */
    private readonly object _sseLock = new();
    private readonly List<ChannelWriter<TelemetrySnapshot>> _sseClients = new();
    ...
    private ProgramProgressSnapshot _programProgress = new();
    private const int MaxHistory = 100;
    private const int MaxAlarmHistory = 50;

    /* High-res graph data streamed from STM32 */
    private readonly List<GraphSamplePoint> _graphPending = new();
    private GraphDataResponse? _graphData;
    private bool _graphReceiving;
    private int  _graphExpect;

    private readonly GraphDb _graphDb;

    public UartGateway(GraphDb graphDb)
    {
        _graphDb = graphDb;
    }
        ...
        lock (_sync)
        {
            return new UartStatusResponse
            {
                ...
                Telemetry = new TelemetrySnapshot
                {
                    ...
                    PositionHw   = _telemetry.PositionHw,
                ...
                Settings = new SettingsSnapshot
                {
                    ...
                    Program = new ProgramSettings
                    {
                        ...
                        Delay1Ms = _settings.Program.Delay1Ms,
                        Delay2Ms = _settings.Program.Delay2Ms,
                        Delay3Ms = _settings.Program.Delay3Ms,
                        RepeatCount = _settings.Program.RepeatCount,
                        InterCycleDelayMs = _settings.Program.InterCycleDelayMs,
                    }
                },
                ...
                ProgramProgress = new ProgramProgressSnapshot
                {
                    Current = _programProgress.Current,
                    Total   = _programProgress.Total,
                },
            };
        }
    }

    public IAsyncEnumerable<TelemetrySnapshot> SubscribeTelemetry(CancellationToken ct)
    {
        var ch = Channel.CreateBounded<TelemetrySnapshot>(
            new BoundedChannelOptions(1) { FullMode = BoundedChannelFullMode.DropOldest });

        lock (_sseLock) { _sseClients.Add(ch.Writer); }

        ct.Register(() =>
        {
            ch.Writer.TryComplete();
            lock (_sseLock) { _sseClients.Remove(ch.Writer); }
        });
    ...
    public GraphDataResponse GetGraphData()
    {
        lock (_sync)
        {
            return _graphData ?? new GraphDataResponse { Ready = false };
        }
    }
    ...
    private void DrainReceivedLinesNoLock()
    {
        ...
        foreach (var rawLine in lines)
        {
            ...
            /* Suppress graph data lines (pos,torque,step) from log to avoid flooding.
             * GRFS header and GRFE footer are still logged. */
            bool isGraphDataLine = _graphReceiving &&
                                   !line.StartsWith("GRFE", StringComparison.OrdinalIgnoreCase);
            ...
            if (parsed.HasKinematics)
            {
                ...
                _telemetry.PositionHw = parsed.PositionHw;
            ...
            if (parsed.HasKinematics)
            {
                ...
                lock (_sseLock)
                {
                    foreach (var w in _sseClients) w.TryWrite(snap);
                }
            }
        }
    }
    ...
    private bool TryApplyOutgoingProgramKey(string key, string value)
    {
        ...
        if (string.Equals(key, "prog_delay1_ms", StringComparison.OrdinalIgnoreCase)) { _settings.Program.Delay1Ms = n; return true; }
        if (string.Equals(key, "prog_delay2_ms", StringComparison.OrdinalIgnoreCase)) { _settings.Program.Delay2Ms = n; return true; }
        if (string.Equals(key, "prog_delay3_ms", StringComparison.OrdinalIgnoreCase)) { _settings.Program.Delay3Ms = n; return true; }
        if (string.Equals(key, "prog_repeat_count", StringComparison.OrdinalIgnoreCase)) { _settings.Program.RepeatCount = Math.Max(1, Math.Min(99, n)); return true; }
        if (string.Equals(key, "prog_inter_cycle_ms", StringComparison.OrdinalIgnoreCase)) { _settings.Program.InterCycleDelayMs = Math.Max(0, n); return true; }
        return false;
    }
    ...
    private bool TryParseConfigSnapshot(string line)
    {
        ...
        if (trimmed.StartsWith("CFGR,", StringComparison.OrdinalIgnoreCase))
        {
            ...
            _settings.Program.Delay1Ms = ReadMapInt(map, _settings.Program.Delay1Ms, "d1", "delay1");
            _settings.Program.Delay2Ms = ReadMapInt(map, _settings.Program.Delay2Ms, "d2", "delay2");
            _settings.Program.Delay3Ms = ReadMapInt(map, _settings.Program.Delay3Ms, "d3", "delay3");
            _settings.Program.RepeatCount = ReadMapInt(map, _settings.Program.RepeatCount, "rc", "repeat_count");
            _settings.Program.InterCycleDelayMs = ReadMapInt(map, _settings.Program.InterCycleDelayMs, "ic", "inter_cycle_ms");
            _settings.TimestampUtc = DateTime.UtcNow;
            return true;
        }

        if (trimmed.StartsWith("PRGS,", StringComparison.OrdinalIgnoreCase))
        {
            var map = ParseKeyValueMap(trimmed[5..]);
            _programProgress.Current = ReadMapInt(map, 0, "cur");
            _programProgress.Total   = ReadMapInt(map, 0, "total");
            return true;
        }
    ...
    private bool TryParseGraphFrame(string line)
    {
        /* GRFS,n=N — graph stream start */
        if (line.StartsWith("GRFS,", StringComparison.OrdinalIgnoreCase))
        {
            var map = ParseKeyValueMap(line[5..]);
            _graphExpect = (int)ReadMapLong(map, 0L, "n");
            _graphPending.Clear();
            _graphReceiving = true;
            _graphData = new GraphDataResponse { Ready = false };
            return true;
        }

        /* GRFE — graph stream end */
        if (line.Equals("GRFE", StringComparison.OrdinalIgnoreCase))
        {
            var pts = _graphPending.ToArray();
            _graphData = new GraphDataResponse
            {
                Ready       = true,
                CycleNumber = _lastResult.CycleNumber,
                Count       = pts.Length,
            ...
            _graphPending.Clear();
            _graphReceiving = false;
            AddLog($"[GRF] Graph received: {_graphData.Count} samples → saving to DB");

            /* Save to DB asynchronously (outside lock) */
            var snap = _lastResult;
            Task.Run(() =>
            {
                try
                {
                    var id = _graphDb.SaveCycle(snap, pts);
                    lock (_sync) { AddLog($"[GRF] Saved to DB: id={id} cycle={snap.CycleNumber} pts={pts.Length}"); }

                    /* Anomaly detection against active baseline */
                    var baseline = _graphDb.GetActiveBaseline();
                    if (baseline is not null)
                    {
                        var det = AnomalyDetector.Analyze(baseline, pts);
                        _graphDb.UpdateCycleAnomaly(id, det.Type, det.Score);
                        lock (_sync) { AddLog($"[GRF] Anomaly: {det.Type} score={det.Score:F1}"); }
                    }
                }
                catch (Exception ex)
                {
                    lock (_sync) { AddLog($"[GRF] DB save failed: {ex.Message}"); }
                }
            });
        ...
        /* Data line: "ms,pos,torque,velocity,step" — only while receiving */
        if (_graphReceiving)
        {
            var parts = line.Split(',');
            if (parts.Length == 5 &&
                int.TryParse(parts[0].Trim(), NumberStyles.Integer, CultureInfo.InvariantCulture, out var ms)       &&
                int.TryParse(parts[1].Trim(), NumberStyles.Integer, CultureInfo.InvariantCulture, out var pos)      &&
                int.TryParse(parts[2].Trim(), NumberStyles.Integer, CultureInfo.InvariantCulture, out var torque)   &&
                int.TryParse(parts[3].Trim(), NumberStyles.Integer, CultureInfo.InvariantCulture, out var velocity) &&
                int.TryParse(parts[4].Trim(), NumberStyles.Integer, CultureInfo.InvariantCulture, out var step))
            {
                _graphPending.Add(new GraphSamplePoint
                {
                    Ms = ms, Pos = pos, Torque = torque, Velocity = velocity, Step = step
                });
                var cnt = _graphPending.Count;
                if (cnt == 1 || cnt == 10 || cnt % 500 == 0)
                    AddLog($"[GRF] rx={cnt}/{_graphExpect} [{ms},{pos},{torque},{velocity},{step}]");
                return true;
            }
            /* Parse failed — log first few to diagnose format issues */
            if (_graphPending.Count == 0)
                AddLog($"[GRF] pfail parts={parts.Length}: {line[..Math.Min(40, line.Length)]}");
            return true; /* consume to prevent mis-routing */
        }
    ...
    private static bool TryParseTelTelemetry(string line, out ParsedTelemetry parsed)
    {
        ...
        var posHw = ReadMapDouble(map, "posHw", "pos_hw", "position_hw");
        ...
        {
            Position   = double.IsNaN(pos)   ? 0.0 : pos,
            PositionHw = double.IsNaN(posHw) ? 0.0 : posHw,
            Velocity   = double.IsNaN(vel)   ? 0.0 : vel,
            Torque     = double.IsNaN(tq)    ? 0.0 : tq,
