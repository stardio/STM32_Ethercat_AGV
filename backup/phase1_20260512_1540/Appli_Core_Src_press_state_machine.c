...
static const PressConfig_t k_default_config = {
    ...
    .contact_speed      = 50,
    .contact_torque_th  = 10U,      /* 10% torque = contact detected */
    .press_speed        = 100,
...
static uint8_t psm_is_drive_ready(void)
{
    ...
    return (((sw & 0x006FU) == 0x0027U) && (SOEM_GetRunEnable() != 0U) &&
...
static uint8_t psm_is_target_reached(int32_t target_user)
{
    ...
    return (delta <= 200LL) ? 1U : 0U;  /* 200 user-unit tolerance */
}

static uint16_t psm_get_torque_pct(void)
{
    int16_t t = SOEM_GetTorqueActual();
    if (t < 0) { t = (int16_t)(-t); }
    return (uint16_t)t;
}
...
{
    int32_t end_pos    = SOEM_GetPositionActual();
...
static void psm_record_result(JudgeResult_t res)
{
    ...
    g_last_result.end_position  = SOEM_GetPositionActual();
...
static void psm_tick_approach(void)
{
    ...
    {
        SOEM_SetProfileVelocity(g_config.contact_speed);
...
static void psm_tick_contact(void)
{
    ...
    if (torque >= g_config.contact_torque_th)
    {
        ...
        SOEM_SetProfileVelocity(g_config.press_speed);
...
static void psm_tick_dwell(void)
{
    ...
    if (elapsed >= g_config.dwell_time_ms)
    {
        ...
        SOEM_SetProfileVelocity(g_config.return_speed);
        SOEM_SetTorqueLimitPercent(30U);  /* low torque on return */
...
uint8_t PressStateMachine_CycleStart(void)
{
    ...
    SOEM_SetProfileVelocity(g_config.approach_speed);
    SOEM_SetTorqueLimitPercent(g_config.press_max_force);
