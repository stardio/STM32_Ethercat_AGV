...
static uint8_t flash_write_store(void)
{
    ...
    if ((sizeof(RecipeStore_t) % 16U) != 0U)
    {
        /* Pad check: the compiler will likely complain if this is wrong */
        return 0U;
    }
    ...
    const uint32_t *src = (const uint32_t *)&g_store;
    uint32_t dst_addr = RECIPE_FLASH_ADDR;
    uint32_t words    = sizeof(RecipeStore_t) / 16U;  /* 128-bit flash words */
...
static void load_defaults(void)
{
    ...
    r->approach_pos     = 50;
    r->contact_speed    = 50;
    r->contact_torque_th = 10U;
    r->press_speed      = 100;
    r->press_target_pos = 0;
    r->press_max_force  = 80U;
