...
public sealed class UartGateway : IDisposable
{
            ...
            return new UartStatusResponse
            {
                ...
                Settings = new SettingsSnapshot
                {
                    ...
                    Parameter = new ParameterSettings
                    {
                        ...
                        UnitScale = _settings.Parameter.UnitScale,
    ...
    private void ApplyOutgoingCommandNoLock(string command)
    {
        ...
        if (TryApplyOutgoingParameterKey(key, value))
        {
            touched = true;
        }

        if (TryApplyOutgoingProgramKey(key, value))
        {
            touched = true;
        }
    ...
    private bool TryApplyOutgoingParameterKey(string key, string value)
    {
        ...
        if (string.Equals(key, "param_unit_scale", StringComparison.OrdinalIgnoreCase)) { _settings.Parameter.UnitScale = n; return true; }
    ...
    private bool TryParseConfigSnapshot(string line)
    {
        ...
        if (trimmed.StartsWith("CFGP,", StringComparison.OrdinalIgnoreCase))
        {
            ...
            _settings.Parameter.UnitScale = ReadMapInt(map, _settings.Parameter.UnitScale, "unit", "unit_scale");
    ...
    {
        if (!line.StartsWith("RST,", StringComparison.OrdinalIgnoreCase)) { return false; }

        var map = ParseKeyValueMap(line[4..]);
        _lastResult.CycleNumber  = ReadMapLong(map, _lastResult.CycleNumber, "cycle");
        _lastResult.Result       = map.GetValueOrDefault("result", _lastResult.Result);
        _lastResult.PeakForcePct = (int)ReadMapLong(map, _lastResult.PeakForcePct, "force");
        _lastResult.EndPosition  = ReadMapLong(map, _lastResult.EndPosition, "pos");
        _lastResult.CycleTimeMs  = ReadMapLong(map, _lastResult.CycleTimeMs, "ms");
        _lastResult.TimestampUtc = DateTime.UtcNow;
        return true;
    }

    private bool TryParseAlarmFrame(string line)
    {
        if (!line.StartsWith("ALM,", StringComparison.OrdinalIgnoreCase)) { return false; }

        var map = ParseKeyValueMap(line[4..]);
        _activeAlarm.Code    = (int)ReadMapLong(map, _activeAlarm.Code, "code");
        _activeAlarm.Message = map.GetValueOrDefault("msg", _activeAlarm.Message);
        _activeAlarm.Acked   = ReadMapBool(map, "ack") ?? _activeAlarm.Acked;
