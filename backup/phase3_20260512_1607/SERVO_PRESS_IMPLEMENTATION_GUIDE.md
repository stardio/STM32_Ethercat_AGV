...
#### cycle_logger (Phase 2에서 기본 구현 완료)
...
- [ ] CycleLogger_SerializeCSV() → Web 쪽 버퍼 기반으로 변경
- [ ] history_read=N CMD → 최근 N개 RST 프레임 전송 (C# 버퍼링)

#### alarm_manager (Phase 2에서 기본 구현 완료)
- [x] `alarm_manager.h` / `alarm_manager.c` 파일 생성
- [x] AlarmCode_t enum 정의
- [x] AlarmRecord_t 구조체 정의
- [x] AlarmManager_Raise() 구현 — 알람 발생 + 모드 전환
- [x] AlarmManager_Ack() 구현
- [x] AlarmManager_Reset() 구현
- [x] alarm_ack / alarm_reset CMD 핸들러 추가
- [x] ALM,... 응답 프레임 전송
- [ ] LCD Alarm 화면 연결 ← Phase 4

#### Web UI 확장
- [ ] C# `_history` 링 버퍼 (최근 100개 CycleResultSnapshot) 추가
- [ ] C# `_alarmHistory` 리스트 추가
- [ ] `/api/history` 엔드포인트 추가
- [ ] `/api/history/csv` 엔드포인트 추가
- [ ] `/api/alarm/list` 엔드포인트 추가
- [ ] `/api/alarm/ack` / `/api/alarm/reset` 엔드포인트 추가
- [ ] Quality Trend 탭 추가 (Chart.js OK/NG 추이)
- [ ] Alarm History 탭 추가
