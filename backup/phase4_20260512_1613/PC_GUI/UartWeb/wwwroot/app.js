...
function applySettingsToForm(settings) {
  ...
  setInputValue(els.pUnitScale, param.unitScale ?? 1);
...
function sendJogDelta(dir) {
  const step = Math.max(1, toInt(els.pJogSpeed, defaultJogStepPuls));
...
function bindParameter() {
  const sendParameterValues = async () => {
    ...
    await sendCmd(`CMD,param_unit_scale=${Math.max(1, toInt(els.pUnitScale))}`);
...
async function loadQualityHistory() {
  try {
    const resp = await api('/api/history?n=100');
...
async function loadAlarmHistory() {
  try {
    const entries = await api('/api/alarm/list');
...
function bindMaintenance() {
  if (els.loginBtn) {
    els.loginBtn.addEventListener('click', async () => {
      ...
      try {
        await api('/api/user/login', 'POST', { pin: parseInt(pin, 10) });
        await pollStatus();
      } catch (err) {
        alertError(err);
      }
