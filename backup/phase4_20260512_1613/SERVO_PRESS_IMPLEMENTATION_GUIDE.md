...
### Phase 2 — Recipe + Quality ✅ 완료 (2026-05-12)

백업: `backup/phase2_20260512_1557/`

#### recipe_manager
- [x] `recipe_manager.h` / `recipe_manager.c` 파일 생성
- [x] RecipeData_t 구조체 정의 (magic=0xA55A, CRC16/CCITT-FALSE 포함)
- [x] RecipeManager_Init() — Flash Sector 6 로드 + 검증, 기본값 슬롯 0 "Default" 초기화
- [x] RecipeManager_Load() / Save() 구현 (QUADWORD 16-byte 정렬 Flash 쓰기)
- [x] RecipeManager_SetActive() 구현
- [x] RecipeManager_Clone() 구현
- [x] RecipeManager_Lock() / Unlock() 구현
- [x] recipe_select / recipe_read / recipe_apply / recipe_clone / recipe_lock / recipe_list / recipe_save_active CMD 핸들러 추가
- [x] NVM 레시피 영역 FLASH_SECTOR_NB-2 (Sector 6) 정의

#### quality_judge (press_state_machine에 통합)
- [x] JudgeResult_t enum 정의 (OK/NG_FORCE_HIGH/LOW/POS_HIGH/LOW/TIME_OVER/INTERLOCK/ABORT)
- [x] 품질 판정 4항목 평가 로직 (press_state_machine.c 내 인라인)
- [x] CYCLE_END 상태에서 평가 호출
- [x] RST,cycle=N,result=OK|NG_xxx,force=%u,pos=%d,ms=%u 응답 프레임 전송

#### 생산 카운터 (PressCounter_t — press_state_machine.h)
- [x] PressCounter_t 구조체 정의 (total/ok/ng/ng별 종류/consecutive_ng/ng_rate)
- [x] 카운터 증감 함수 구현 (CYCLE_END/CYCLE_NG 전환 시 자동 갱신)
- [x] NG Rate 계산 (ng_rate_x10 × 10 = 백분율)
- [x] 연속 NG 카운터 + AlarmManager_Raise(ALARM_CONSECUTIVE_NG) 연동
- [x] CNT,total=%u,ok=%u,ng=%u,cng=%u,rate=%u.%u 응답 프레임 추가

#### cycle_logger + alarm_manager (Phase 3 전반부 선행 구현)
- [x] `cycle_logger.h` / `cycle_logger.c` 파일 생성 (RAM 링 버퍼 100개)
- [x] `alarm_manager.h` / `alarm_manager.c` 파일 생성 (RAM 이력 20개)
- [x] ALM,code=%u,msg=%s,ack=%u 응답 프레임 추가
- [x] CMakeLists.txt에 5개 모듈 추가 (빌드 성공, Flash 25.64%)

#### Web (UartGateway / ApiModels)
- [x] PressStatusSnapshot / ProductionCounterSnapshot / CycleResultSnapshot / AlarmSnapshot 모델 추가
- [x] UartGateway PST/CNT/RST/ALM 프레임 파서 추가
- [x] GetStatus() 응답에 4개 스냅샷 추가
- [x] Web 빌드 성공 (경고 0, 오류 0)

#### LCD Auto Run / Recipe 화면
- [ ] TouchGFX Designer에 Auto Run 화면 추가 ← **Phase 4에서 진행**
- [ ] TouchGFX Designer에 Recipe 화면 추가 ← **Phase 4에서 진행**

---

### Phase 3 — Traceability + Alarm ✅ 완료 (2026-05-12)

백업: `backup/phase3_20260512_1607/`

#### cycle_logger
- [x] `cycle_logger.h` / `cycle_logger.c` 파일 생성
- [x] CycleRecord_t 구조체 정의
- [x] 링 버퍼 구현 (100개, RAM)
- [x] CycleLogger_Record() — CYCLE_END/CYCLE_NG 시 호출
- [x] CycleLogger_GetLast() 구현
- [x] CycleLogger_SerializeCSV() (firmware 사이드)
- [x] history_read=N CMD → 최근 N개 HSTI 프레임 전송 (judge 이름 문자열)
- [x] alarm_history_read=N CMD → 최근 N개 ALME 프레임 전송

#### alarm_manager
- [x] `alarm_manager.h` / `alarm_manager.c` 파일 생성
- [x] AlarmCode_t enum 정의 (100/200/300/400/500번대)
- [x] AlarmRecord_t 구조체 정의 (occurred_ms/cleared_ms/ack_done)
- [x] AlarmManager_Raise() — 알람 발생 + PressStateMachine_RaiseAlarm()
- [x] AlarmManager_Ack() / AlarmManager_Reset() 구현
- [x] alarm_ack / alarm_reset CMD 핸들러 추가
- [x] ALM,code=%u,msg=%s,ack=%u 응답 프레임 전송
- [x] AlarmManager_GetHistoryEntry() — 이력 조회

#### Web UI 확장 (UartGateway + Program.cs + HTML/JS/CSS)
- [x] C# `_history` Queue<CycleResultSnapshot> (max 100) 추가
- [x] C# `_alarmHistory` Queue<AlarmHistoryEntry> (max 50) 추가
- [x] RST/HSTI 프레임 파서 → `_history` 푸시
- [x] ALM/ALME 프레임 파서 → `_alarmHistory` 푸시
- [x] `/api/history?n=N` 엔드포인트 추가
- [x] `/api/history/csv` CSV 다운로드 엔드포인트 추가
- [x] `/api/history/clear` POST 엔드포인트 추가
- [x] `/api/alarm/list` 엔드포인트 추가
- [x] `/api/alarm/ack` / `/api/alarm/reset` POST 엔드포인트 추가
- [x] `/api/alarm/fetch` (alarm_history_read CMD 전송) 추가
- [x] 웹 타이틀 "Servo Press Web Control"로 변경
- [x] Press Status Bar — Mode/Step/인터록 LED/생산카운터/ActiveAlarm 실시간 표시
- [x] Press Control 탭 추가 — 모드전환/사이클시작/파라미터/판정기준/시뮬레이션/결과박스
- [x] Quality Trend 탭 추가 — Chart.js OK/NG 막대 차트 + 이력 테이블 + CSV 다운로드
- [x] Alarm History 탭 추가 — ACK/Reset 버튼 + 이력 테이블
- [x] Web 빌드 성공 (경고 0, 오류 0)
