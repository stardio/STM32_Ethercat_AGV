...
enum
{
  PC_PARAM_JOG_SPEED = 0,  /* index 0 */
  PC_PARAM_ACC_TIME,        /* index 1 — from drive SDO values[1] */
  PC_PARAM_DEC_TIME,        /* index 2 — from drive SDO values[2] */
  PC_PARAM_LIMIT_PLUS,      /* index 3 — from drive SDO values[3] */
  PC_PARAM_LIMIT_MINUS,     /* index 4 — from drive SDO values[4] */
  PC_PARAM_GEAR_RATIO,      /* index 5 — 감속비 (integer, e.g. 5 for 5:1) */
  PC_PARAM_HOME_OFFSET,     /* index 6 */
  PC_PARAM_POSITION_GAIN,   /* index 7 — from drive SDO values[7] */
  PC_PARAM_BALL_LEAD,       /* index 8 — 볼스크류 리드 (mm, integer) */
  PC_PARAM_ENC_RES,         /* index 9 — 엔코더 분해능 (counts/rev) */
  PC_PARAM_COUNT            /* = 10 */
};
...
static int32_t Pc_GetUnitScale(void);       /* forward declaration */
static void Pc_ApplyParametersToDrive(void); /* forward declaration */
...
static void Pc_HomeDiagLog(const char *tag)
{
  ...
  Pc_CmdReply("[HOME_DIAG] tag=%s cur_u=%ld cur_hw=%ld home_hw=%ld home_user=%ld unit=%ld",
              ...
              (long)Pc_GetUnitScale());
}

/* Compute effective unit scale (counts/mm) from gear ratio, ball screw lead, encoder resolution.
 * scale = enc_res * gear_ratio / ball_lead_mm
 * Returns 1 if any parameter is zero/unset (safe default = raw counts). */
static int32_t Pc_GetUnitScale(void)
{
  int64_t gear = (int64_t)g_pcCmd.parameterValues[PC_PARAM_GEAR_RATIO];
  int64_t lead = (int64_t)g_pcCmd.parameterValues[PC_PARAM_BALL_LEAD];
  int64_t enc  = (int64_t)g_pcCmd.parameterValues[PC_PARAM_ENC_RES];
  int64_t scale;

  if (gear <= 0LL) { gear = 1LL; }
  if (lead <= 0LL) { lead = 1LL; }
  if (enc  <= 0LL) { enc  = 1LL; }

  scale = (enc * gear) / lead;
  if (scale <= 0LL) { scale = 1LL; }
  if (scale > (int64_t)INT32_MAX) { scale = (int64_t)INT32_MAX; }
...
static void Pc_ReplyConfigSnapshot(void)
{
  ...
  Pc_CmdReply("CFGP,jog=%ld,acc=%ld,dec=%ld,lplus=%ld,lminus=%ld,gear=%ld,lead=%ld,enc=%ld,home=%ld,gain=%ld",
              ...
              (long)g_pcCmd.parameterValues[PC_PARAM_GEAR_RATIO],
              (long)g_pcCmd.parameterValues[PC_PARAM_BALL_LEAD],
              (long)g_pcCmd.parameterValues[PC_PARAM_ENC_RES],
              (long)g_pcCmd.parameterValues[PC_PARAM_HOME_OFFSET],
...
static void Pc_SyncTouchGfxModel(void)
{
  ...
  /* LCD 모델이 flash[5](gear_ratio)를 unit_scale로 오해하여 SOEM을 덮어쓸 수 있으므로
   * 웹UI 기준값으로 재적용. */
...
static void Pc_ApplyParametersToDrive(void)
{
  ...
  if (jogSpeed < 0) { jogSpeed = -jogSpeed; }
  if (jogSpeed <= 0) { jogSpeed = 1; }
  if (acc < 0) { acc = -acc; }
  if (dec < 0) { dec = -dec; }
  if (posGain < 0) { posGain = 0; }
  ...
  SOEM_SetUnitScale(Pc_GetUnitScale());
...
static void Pc_CommandInit(void)
{
  ...
  g_pcCmd.parameterValues[PC_PARAM_GEAR_RATIO] = 1;
  g_pcCmd.parameterValues[PC_PARAM_BALL_LEAD]  = 1;
  g_pcCmd.parameterValues[PC_PARAM_ENC_RES]    = 1;
  ...
  /* Apply all flash-loaded parameters to SOEM immediately so unit_scale,
   * velocity, limits, etc. are correct before EtherCAT goes operational.
   * Without this, unit_scale stays 1 (counts) until the user presses Write All. */
  Pc_ApplyParametersToDrive();
}
...
static void Pc_ProcessCommandLine(char *line)
{
  ...
  *eq = '\0';
  ...
  if (Pc_StrEqIgnoreCase(key, "jog_velocity"))
  {
    parsed = Pc_ParseInt32(value, 0);
    if (Pc_IsMotionEnabled() != 0U)
    {
      if (parsed > 0)
      {
        SOEM_SetTargetPositionAbs(g_pcCmd.parameterValues[PC_PARAM_LIMIT_PLUS]);
      }
      else if (parsed < 0)
      {
        SOEM_SetTargetPositionAbs(g_pcCmd.parameterValues[PC_PARAM_LIMIT_MINUS]);
      }
      else
      {
        SOEM_SetTargetPositionAbs(SOEM_GetPositionActual());
  ...
  if (Pc_StrEqIgnoreCase(key, "set_home"))
  {
    ...
    {
      int32_t unitScale = Pc_GetUnitScale();
  ...
  if (Pc_StrEqIgnoreCase(key, "param_gear_ratio"))        { g_pcCmd.parameterValues[PC_PARAM_GEAR_RATIO] = Pc_ParseInt32(value, g_pcCmd.parameterValues[PC_PARAM_GEAR_RATIO]); SOEM_SetUnitScale(Pc_GetUnitScale()); return; }
  if (Pc_StrEqIgnoreCase(key, "param_lead"))              { g_pcCmd.parameterValues[PC_PARAM_BALL_LEAD]  = Pc_ParseInt32(value, g_pcCmd.parameterValues[PC_PARAM_BALL_LEAD]);  SOEM_SetUnitScale(Pc_GetUnitScale()); return; }
  if (Pc_StrEqIgnoreCase(key, "param_enc_res"))           { g_pcCmd.parameterValues[PC_PARAM_ENC_RES]    = Pc_ParseInt32(value, g_pcCmd.parameterValues[PC_PARAM_ENC_RES]);    SOEM_SetUnitScale(Pc_GetUnitScale()); return; }
  ...
  {
    int32_t req_n = Pc_ParseInt32(value, 0);
    if (req_n <= 0) { req_n = 1; }
    if (req_n > (int32_t)CYCLE_LOG_MAX) { req_n = (int32_t)CYCLE_LOG_MAX; }
    uint16_t cnt = CycleLogger_GetCount();
    uint16_t rows = (uint16_t)req_n;
    if (rows > cnt) { rows = cnt; }
    Pc_CmdReply("HIST,count=%u,total=%u", (unsigned int)rows, (unsigned int)cnt);
    static const char *judge_names[] = {
        "OK","NG_FORCE_HIGH","NG_FORCE_LOW","NG_POS_HIGH",
        "NG_POS_LOW","NG_TIME_OVER","NG_INTERLOCK","NG_ABORT"
    };
    for (uint16_t hi = 0U; hi < rows; hi++)
    {
      const CycleRecord_t *rec = CycleLogger_GetLast((uint8_t)hi);
      if (rec == NULL) { break; }
      uint8_t ri = (rec->result < 8U) ? (uint8_t)rec->result : 0U;
      Pc_CmdReply("HSTI,n=%lu,result=%s,force=%u,pos=%ld,ms=%lu",
  ...
  if (Pc_StrEqIgnoreCase(key, "alarm_history_read"))
  {
    int32_t req_n = Pc_ParseInt32(value, 0);
    if (req_n <= 0) { req_n = 1; }
    uint8_t cnt_ah = AlarmManager_GetHistoryCount();
    uint8_t rows_ah = (uint8_t)req_n;
    if (rows_ah > cnt_ah) { rows_ah = cnt_ah; }
    Pc_CmdReply("ALMH,count=%u", (unsigned int)rows_ah);
    for (uint8_t ai = 0U; ai < rows_ah; ai++)
    {
      const AlarmRecord_t *ar = AlarmManager_GetHistoryEntry(ai);
      if (ar == NULL) { break; }
      Pc_CmdReply("ALME,code=%u,msg=%s,ack=%u,ms=%lu",
                  (unsigned int)ar->code,
                  ar->message,
                  (unsigned int)ar->ack_done,
                  (unsigned long)ar->occurred_ms);
    }
    return;
  }

  /* ---- Phase 4: User Auth ---- */
  if (Pc_StrEqIgnoreCase(key, "user_login"))
  {
    int32_t pin = Pc_ParseInt32(value, -1);
    if (pin >= 0)
    {
      if (UserAuth_Login((uint16_t)pin) != 0U)
      {
        UserAuth_ReplyLevel(Pc_CmdReply);
      }
      else
      {
        Pc_CmdReply("[CMD] user_login failed: wrong PIN");
      }
    }
    return;
  }

  if (Pc_StrEqIgnoreCase(key, "user_logout"))
  {
    UserAuth_Logout();
    Pc_CmdReply("ULV,level=0,name=OPERATOR");
    return;
  }

  if (Pc_StrEqIgnoreCase(key, "user_level"))
  {
    UserAuth_ReplyLevel(Pc_CmdReply);
    return;
  }

  /* ---- Phase 4: Maintenance Counter ---- */
  if (Pc_StrEqIgnoreCase(key, "maint_read"))
  {
    MaintCounter_Reply(Pc_CmdReply);
    return;
  }

  if (Pc_StrEqIgnoreCase(key, "maint_reset"))
  {
    if (UserAuth_IsAtLeast(USER_LEVEL_ENGINEER) != 0U)
    {
      MaintCounter_Reset();
      Pc_CmdReply("[CMD] maint_reset ok");
      MaintCounter_Reply(Pc_CmdReply);
    }
    else
    {
      Pc_CmdReply("[CMD] maint_reset denied: Engineer level required");
    }
    return;
  }

  if (Pc_StrEqIgnoreCase(key, "maint_thresh"))
  {
    if (UserAuth_IsAtLeast(USER_LEVEL_ENGINEER) != 0U)
    {
      int32_t thr = Pc_ParseInt32(value, 0);
      if (thr > 0)
      {
        MaintCounter_SetThreshold((uint32_t)thr);
        Pc_CmdReply("[CMD] maint_thresh=%ld ok", (long)thr);
      }
    }
    else
    {
      Pc_CmdReply("[CMD] maint_thresh denied: Engineer level required");
    }
    return;
  }
...
static void Pc_CommandTick(void)
{
  if (g_pcCmd.parameterReadPending != 0U)
  {
    ...
    if (SOEM_FetchParameterReadAll(values, UI_FLASH_PARAMETER_VALUE_COUNT) != 0U)
    {
      ...
      /* values[5] was unit_scale (deprecated) — gear/lead/enc are STM32-local params */
...
void EtherCAT_Task(void *argument)
{
  ...
  MaintCounter_Init();
  RecipeManager_ApplyActive();
  UART4_SendText("[PSM] Press SM + Recipe + Logger + Alarm + Auth + Maint initialized\r\n");
