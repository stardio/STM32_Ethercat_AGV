...
static void soem_apply_pending_motion_settings(void)
{
  ...
  if (soem_parameter_read_all_pending != 0U)
  {
    ...
    /* Home offset is always known from soem_home_offset (set by OGR Reset or loaded at boot).
       Do not read 0x607C directly as it may differ from the value actually configured in SOEM.
       Use soem_home_offset which represents the current configured home offset calibration. */
    int okHome = (soem_home_offset != 0) ? 1 : 1;  /* Home offset always valid */
        ...
        (okLimitMinus > 0) && (okLimitPlus > 0) && (okGain > 0))
    {
      ...
      /* Use soem_home_offset instead of reading 0x607C, to preserve calibration value */
      soem_parameter_read_all_values[6] = soem_home_offset / unitScaleUser;
