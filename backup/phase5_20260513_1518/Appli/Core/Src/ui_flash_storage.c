...
static uint8_t UiFlash_WriteLayout(const UiFlashInternalLayout* layout)
{
    ...
    /* Firmware executes from external XSPI flash (0x70000000), not internal NVM.
     * Internal NVM erase does not affect instruction fetch, so disabling IRQs
     * is unnecessary and would block the EtherCAT interrupt. */
